...
useAnthropic = True
agentModel = "claude-opus-4-6"  # "claude-haiku-4-5"
    ...
    for filename in os.listdir(dataPath):
        if filename == evaluating_testcase_name:
            ...
            if os.path.isfile(filepath) and os.path.isfile(solutionPath):
                for coalition in getPromptingMatrix(promptingStragegies)[:2]:
...
async def _run_manual_agent(
    ...
    if not (not coalition.coalitionPrompt):
        messages.append({"role": "system", "content": coalition.coalitionPrompt})

    if useAnthropic:
        ai_agent = ClaudeAgent(
            messages=messages, max_number_of_turns=25, model=agentModel
        )
    else:
